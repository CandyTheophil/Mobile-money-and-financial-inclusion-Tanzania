...
with open(
    join(dirname(realpath(__file__)), "Model/group7_model.pkl"), "rb",) as fm :
    model = joblib.load(fm)

with open(join(dirname(realpath(__file__)), "Pre-processing/MinMaxScaler.pkl"), "rb") as m:
    scaler = joblib.load(m)


with open(
    join(dirname(realpath(__file__)), "Pre-processing/one-hot-encoder.pkl"), "rb") as o:
